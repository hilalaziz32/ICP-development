...
This is the **evergreen-grounded copy path** in the SMS playbook. The API below is the
live, runtime face of `sms-playbook/evergreen-db-spec.md` — the same niche-tagged,
reply-scored memory, exposed as endpoints. It pulls the *data*; the playbook (below)
supplies the *voice*. Both bind on every run.
...
## Voice gate (light — do this first, every run)

The API gives you the *ammo*; the Voice Profile gives you the *voice*. You write a
**grounded draft** here; final voice polish, lever selection, and QA are `sms-draft`'s
job (the craft layer). So keep this light — **reference, never restate:**

- `sms-playbook/Cold-SMS-Voice-Profile-Scaletopia.md` — voice, the T1/T2 anatomy +
  never-cut spine, the banlist. **The one non-negotiable bar.**
- *(optional gate)* `sms-playbook/FUNDAMENTALS.md` — the two bars ("this is real" +
  "this is for me") and the one job: make the right person think *"tell me more."*

Lever choice and deep QA are **not** done here — they live in `sms-draft` and in the
`copies`/`components` winners the API already returns. Then obey the universal
guardrails: **plain language, no coined jargon**; the **mechanism is a core pillar**
(the HOW — never optional filler); never invent a metric, mechanism, or competitor.
...
0. **Voice gate**: read the Voice Profile (FUNDAMENTALS optional). Write a *grounded draft* to its bar; `sms-draft` does final polish + QA.
...
7. **Write** the copy using: confirmed pain → disarm objection → proof line (mechanism) → soft CTA, in the buyer's lingo, within SMS length, on the Voice Profile anatomy.
...
- Clear the **Voice Profile** bar before saving — plain language, mechanism intact, banlist honored. Data-true is not enough; the draft must also be on-voice.
